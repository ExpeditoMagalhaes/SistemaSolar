...
void introscreen(){
	...
	char buf[100] = { 0 };
	sprintf_s(buf, "Velocidade: [s]low [f]ast");
	renderbitmap(-90, -80, GLUT_BITMAP_TIMES_ROMAN_24, buf); //1280 x 720
	sprintf_s(buf, "Cameras: [1] [2] [3]");
	renderbitmap(-20, -80, GLUT_BITMAP_TIMES_ROMAN_24, buf);  //1280 x 720
	sprintf_s(buf, "Zoom: - MOUSE(1)   + MOUSE(2)");
	renderbitmap(40, -80, GLUT_BITMAP_TIMES_ROMAN_24, buf);  //1280 x 720
}
...
void display(void)
{
	...
	//legendando - mostrar apenas na cam1
	if (changeCamera == 0) {
		introscreen();
	}
